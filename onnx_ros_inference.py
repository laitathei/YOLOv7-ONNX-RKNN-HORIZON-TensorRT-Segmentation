...
class onnx_ros_inference():
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            outputs = self.sess.run(['output', '528', '638', '748', '513'],{"images": image_4c.astype(np.float32)}) # (1, 3, input height, input width)
            outputs = [outputs[0], outputs[4]]
            colorlist = gen_color(len(self.CLASSES))
            results = postprocess(outputs, image_4c, image_3c, self.conf_thres, self.iou_thres) ##[box,mask,shape]
